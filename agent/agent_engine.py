    ...
    def __init__(self, project_id: str = None, location: str = None, model_name: str = None, guide_path: str = None):
        ...
        self.model = GenerativeModel(
            ...
            system_instruction=[
                ...
                "- FLOW: Confirmation -> `finalize_order` (Show Account) -> WAIT -> User Deposits -> `verify_payment`.",
                "- If the user tries to skip ahead, politely bring them back to the current step.",
    ...
    def query(self, message: str, history: List[str] = None):
        ...
        if self.interaction_state == "AWAITING_PAYMENT_PROOF":
            ...
            if msg_lower in ["yes", "y", "예", "네"]:
                self.interaction_state = "ORDERING" # Reset
        ...
        try:
            for _ in range(max_turns):
                # If no candidates, something went wrong
                if not current_response.candidates or len(current_response.candidates) == 0:
                    if settings.DEBUG:
                        print("  [Debug] No candidates in response.")
                    return "Error: No response from model."
                
                # Check content parts safety
